...
trait GameDef:
  ...
  case class Block(b1: Pos, b2: Pos):
    ...
    /** Returns the list of blocks that can be obtained by moving the current
      ...
      */
    def neighbors: List[(Block, Move)] = Move.values
      .map(m =>
        (
          (m match
            case Move.Left  => Block(b1, b2).left
            case Move.Right => Block(b1, b2).right
            case Move.Up    => Block(b1, b2).up
            case Move.Down  => Block(b1, b2).down
          ),
          m
        )
      )
    ...
    /** Returns the list of positions reachable from the current block which are
      * inside the terrain.
      */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter(m =>
      m._1.isLegal
    )
