...
trait Solver extends GameDef:
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = b.legalNeighbors.map(nb => (nb._1, nb._2 :: history)).to(LazyList)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filter(pair => !explored.contains(pair._1))
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = 
            val newNeighbors: LazyList[(Block, List[Move])] = newNeighborsOnly(initial, explored)
            val tryNeighbors = for
              nn <- newNeighbors
              pair <- neighborsWithHistory(nn._1, nn._2)
            yield pair
            initial #::: from(tryNeighbors, explored ++ initial.map(_._1))


  /**
   * The lazy list of all paths that begin at the starting block.
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = neighborsWithHistory(startBlock, List())

  /**
   * Returns a lazy list of all possible pairs of the goal block along
   * with the history how it was reached.
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = from(pathsFromStart, Set(startBlock)).filter((b, _) => done(b))
