...
trait DecoderInstances:
  ...
  given Decoder[Int] = 
    Decoder.fromPartialFunction { case Json.Num(value: BigDecimal) if value.isValidInt => value.intValue}
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(arr: List[Json]) => 
        val l: List[A] = arr.map(value => decoder.decode(value) match 
          case Some(value) => value)
        if l.size.equals(arr.size) then Some(l) else None
      case _ => Option.empty
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(fields) if fields.contains(name) => fields(name).decodeAs[A]
      case _ => null
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    ...
    .transform[Person](tuple => Person(tuple._1, tuple._2))
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] =
    ...
    .transform[Contacts](cs => Contacts(cs))
